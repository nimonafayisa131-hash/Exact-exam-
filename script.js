...
const SK_VISITS = 'eq2_visits';
const SK_LOGIN_ATTEMPTS = 'eq2_login_attempts';

// ===== PASSWORD HASHING =====
// Passwords used to be stored as plain text. They're now stored as
// "sha256:<hex digest>" instead, so a database leak no longer exposes
// anyone's real password. Existing plain-text records still work — see
// passwordMatches() below — and get silently upgraded to a hash the next
// time that person logs in successfully.
...
async function hashPassword(plainPassword) {
  return HASH_PREFIX + await sha256Hex(plainPassword);
}

// Checks a typed-in password against whatever is on file — a hash
// (normal case) or, for records created before hashing existed, plain
// text (legacy case).
async function passwordMatches(enteredPlain, storedValue) {
  if (!storedValue) return false;
  if (storedValue.startsWith(HASH_PREFIX)) {
    return (await hashPassword(enteredPlain)) === storedValue;
  }
  return enteredPlain === storedValue;
}

// ===== LOGIN ATTEMPT LOCKOUT =====
// After 3 wrong passwords in a row, the login form locks for 5 minutes.
// This is enforced in the browser only (there's no login server), so it
// slows down casual guessing but a determined attacker could reset it by
// clearing site data — see the security review for the real fix
// (moving login through a Cloud Function with server-side rate limiting).
const MAX_LOGIN_ATTEMPTS = 3;
const LOGIN_LOCKOUT_MINUTES = 5;

function getLoginAttemptState() {
  try {
    return JSON.parse(localStorage.getItem(SK_LOGIN_ATTEMPTS)) || { count: 0, lockedUntil: 0 };
  } catch (e) {
...
function clearLoginAttempts() {
  localStorage.setItem(SK_LOGIN_ATTEMPTS, JSON.stringify({ count: 0, lockedUntil: 0 }));
}

// Returns how many milliseconds are left on a lockout (0 if not locked).
function getLoginLockRemainingMs() {
  return Math.max(0, (getLoginAttemptState().lockedUntil || 0) - Date.now());
}

// ===== SITE VISIT LOG =====
// Records that someone opened the site, with a timestamp, so the Admin
// can see traffic under Settings -> Site Visits. Keeps only the most
// recent 500 entries.
function logSiteVisit() {
  let visits = [];
  try {
    visits = JSON.parse(localStorage.getItem(SK_VISITS)) || [];
  } catch (e) {
    visits = [];
  }
  visits.push({ time: new Date().toISOString() });
  if (visits.length > 500) visits = visits.slice(visits.length - 500);
  localStorage.setItem(SK_VISITS, JSON.stringify(visits));
  fbGet('visits').then(remote => {
    let merged = Array.isArray(remote) ? remote.slice() : [];
    merged.push({ time: visits[visits.length - 1].time });
    if (merged.length > 500) merged = merged.slice(merged.length - 500);
    localStorage.setItem(SK_VISITS, JSON.stringify(merged));
    fbSet('visits', merged);
  });
}
...
async function fbGet(key, timeoutMs = 6000) {
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    const res = await fetch(`${FIREBASE_URL}/${key}.json`, { signal: controller.signal });
    clearTimeout(timer);
...
// ===== EXAM PROGRESS (cross-browser resume) =====
// Keys in Firebase can't contain '.', '#', '$', '[', ']', or '/'
function sanitizeFbKeyPart(str) {
  return String(str || '').replace(/[.#$[\]/]/g, '_');
}

function examProgressKeyFor(studentId, courseId) {
  return sanitizeFbKeyPart(studentId.toUpperCase()) + '__' + sanitizeFbKeyPart(courseId);
}

function saveExamProgress() {
  if (!examProgressKey) return;
  fbSet('examProgress/' + examProgressKey, {
    studentId: currentStudent.id,
    courseId: currentStudent.courseId,
    courseName: currentStudent.courseName,
    studentName: currentStudent.name,
    dept: currentStudent.dept,
    examQuestions, answerMap, answers, flags, currentQ,
    totalTime: TOTAL_TIME,
    startedAt: examStartedAt,
    tabWarnings: examTabWarnings,
    updatedAt: Date.now()
  });
}
...
function clearExamProgress() {
  if (!examProgressKey) return;
  fetch(`${FIREBASE_URL}/examProgress/${examProgressKey}.json`, { method: 'DELETE' }).catch(() => {});
  examProgressKey = null;
}
...
let examStartedAt = 0;            // wall-clock ms timestamp the exam attempt began
let examProgressKey = null;       // Firebase key for this student+course's in-progress exam
let examTabWarnings = 0;          // how many times the student has left the exam tab
let examTabSwitchPending = false; // true while the tab is hidden, waiting for them to return
...
async function findAuthorizedStudent(username, password) {
  let list = loadAuthorized();
  const u = username.trim().toLowerCase();
  let match = list.find(s => (s.username || '').toLowerCase() === u);
  if (!match) {
    // The local copy might not have synced yet (e.g. admin just added this
    // student on another device/browser, or this device opened the page
    // moments ago and the background Firebase sync hasn't finished). Do
    // one direct, fresh check against Firebase before giving up.
    const remote = await fbGet('authorized');
    if (Array.isArray(remote)) {
      localStorage.setItem(SK_AUTHORIZED, JSON.stringify(remote));
      list = remote;
      match = list.find(s => (s.username || '').toLowerCase() === u);
    }
  }
  if (!match) return null;
  const ok = await passwordMatches(password, match.password);
  if (!ok) return null;
  if (!match.password.startsWith(HASH_PREFIX)) {
    match.password = await hashPassword(password);
    saveAuthorized(list);
  }
  return match;
}
...
async function addAuthorizedStudent() {
  ...
  list.push({ id, name, dept, username, password: await hashPassword(password) });
  saveAuthorized(list);
  idInput.value = '';
  nameInput.value = '';
  deptInput.value = '';
...
async function addAuthorizedBulk() {
...
async function findTeacher(username, password) {
  let list = loadTeachers();
  const u = username.trim().toLowerCase();
  let match = list.find(t => (t.username || '').toLowerCase() === u);
  if (!match) {
    const remote = await fbGet('teachers');
    if (Array.isArray(remote)) {
      localStorage.setItem(SK_TEACHERS, JSON.stringify(remote));
      list = remote;
      match = list.find(t => (t.username || '').toLowerCase() === u);
    }
  }
  if (!match) return null;
  const ok = await passwordMatches(password, match.password);
...
async function addTeacher() {
  ...
  err.classList.remove('show');
  list.push({ id: 'teacher_' + Date.now(), name, username, password: await hashPassword(password), courseIds });
  saveTeachers(list);
  nameInput.value = '';
  userInput.value = '';
  passInput.value = '';
...
// Keeps the header button's icon in sync with the current theme — called
// once at startup (the <html class="dark-mode"> itself is already set by
// the inline <script> in <head>, before this file even loads).
...
async function handleUnifiedLogin() {
  ...
  // Everything below used to run with no try/catch. If any step threw
  // (bad course data, a DOM element missing, a network hiccup mid-await,
  // etc.), the credentials could be 100% correct but the screen would
  // just sit there with no visible error — the exception only showed up
  // in the browser console, invisible on a phone. Now any failure is
  // shown to the person instead of failing silently.
  try {
    loadData();
    ...
    if (lockRemaining > 0) {
      ...
      err.textContent = `Too many failed attempts. Please try again in ${mins} minute${mins === 1 ? '' : 's'}.`;
    ...
    if (username.toLowerCase() === getAdminUsername().toLowerCase() && await passwordMatches(password, getAdminPassword())) {
      ...
      if (!getAdminPassword().startsWith(HASH_PREFIX)) {
        settings.adminAuth = { username: getAdminUsername(), password: await hashPassword(password) };
    ...
    const student = await findAuthorizedStudent(username, password);
    if (student) {
      clearLoginAttempts();
      err.classList.remove('show', 'notice-green');
    ...
    recordFailedLogin();
    const nowLocked = getLoginLockRemainingMs();
    if (nowLocked > 0) {
      const mins = Math.ceil(nowLocked / 60000);
      err.textContent = `Too many failed attempts. Please try again in ${mins} minute${mins === 1 ? '' : 's'}.`;
    } else {
    ...
    err.textContent = 'Something went wrong while logging in (' + e.message + '). Please try again, or tell Admin this exact message.';
    err.classList.remove('notice-green');
    err.classList.add('show');
  }
}
...
async function handleCourseSelectContinue() {
  ...
  // Check whether this student already has this exam in progress (started
  // in another browser/device) and, if so, pick up right where they left off.
  const startBtn = document.getElementById('course-start-btn');
  if (startBtn) { startBtn.disabled = true; startBtn.textContent = 'Checking for saved progress…'; }
  const progressKey = examProgressKeyFor(currentStudent.id, currentStudent.courseId);
  const existingProgress = await fbGet('examProgress/' + progressKey);
  if (startBtn) { startBtn.disabled = false; startBtn.textContent = 'Start Exam →'; }
  startExam(course, existingProgress || null);
}

// ===== EXAM =====
// resumeData (optional): a previously-saved progress record from Firebase,
// used when the student re-opens the exam in a different browser/device.
function startExam(course, resumeData) {
  const submitBtn = document.getElementById('exam-submit-btn');
  if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = '✔ Submit'; }
  const s = getCourseSettings(course);

  if (resumeData) {
    examQuestions = resumeData.examQuestions;
    answerMap = resumeData.answerMap;
    answers = resumeData.answers;
    flags = resumeData.flags || new Array(examQuestions.length).fill(false);
    currentQ = resumeData.currentQ || 0;
    TOTAL_TIME = resumeData.totalTime;
    examStartedAt = resumeData.startedAt;
    examTabWarnings = resumeData.tabWarnings || 0;
  } else {
    currentQ = 0;
  ...
  // Time left is always derived from real elapsed wall-clock time since the
  // attempt started, so it stays correct no matter which browser/device the
  // student is on when they resume.
  timeLeft = Math.max(0, TOTAL_TIME - Math.floor((Date.now() - examStartedAt) / 1000));

  examProgressKey = examProgressKeyFor(currentStudent.id, currentStudent.courseId);
  examTabSwitchPending = false;
  saveExamProgress();

  showScreen('screen-exam');
  document.getElementById('exam-student-label').textContent = currentStudent.courseName;
  renderExamProfileMenu();
  renderQuestion();

  if (timeLeft <= 0) {
    // Time already ran out while they were away — submit immediately.
    submitExam();
    return;
  }
  startTimer();
...
function renderQuestion() {
  ...
  document.getElementById('q-counter').textContent = `Question ${currentQ + 1}`;
  document.getElementById('exam-q-label').textContent = `👤 ${currentStudent.name} — Question ${currentQ + 1} of ${total}`;
  document.getElementById('q-text').textContent = q.q;
  document.getElementById('exam-progress').style.width = `${((currentQ + 1) / total) * 100}%`;
  document.getElementById('q-answer-status').textContent = answers[currentQ] !== null ? 'Answered' : 'Not yet answered';
  document.getElementById('q-answer-status').classList.toggle('answered', answers[currentQ] !== null);
  ...
  if (isTF) {
    document.getElementById('q-options').innerHTML = ['True', 'False'].map((label, dispIdx) => `
      <div class="option-item ${answers[currentQ] === dispIdx ? 'selected' : ''}" onclick="selectAnswer(${dispIdx})">
        ...
        <span class="option-label-letter">${letters[dispIdx]}.</span>
    ...
    document.getElementById('q-options').innerHTML = map.map((origIdx, dispIdx) => `
      <div class="option-item ${answers[currentQ] === dispIdx ? 'selected' : ''}" onclick="selectAnswer(${dispIdx})">
        <span class="option-radio"><span class="option-radio-dot"></span></span>
        <span class="option-label-letter">${letters[dispIdx]}.</span>
        <span class="option-label-text">${q.opts[origIdx]}</span>
      </div>`).join('');
  }
...
// ===== ANTI-CHEATING: leaving the exam tab/window =====
// If a student switches to another browser tab/window (or minimizes, or
// switches apps) while the exam is live, we warn them the first two times.
// The third time, we auto-submit the exam for them.
...
document.addEventListener('visibilitychange', () => {
  ...
  if (document.hidden) {
    examTabSwitchPending = true;
  } else if (examTabSwitchPending) {
    examTabSwitchPending = false;
    handleExamTabSwitch();
  }
});

function handleExamTabSwitch() {
  examTabWarnings++;
  saveExamProgress();
  if (examTabWarnings >= 3) {
    autoSubmitForTabSwitch();
  } else {
    showTabWarningModal(examTabWarnings);
  }
}

function showTabWarningModal(n) {
  document.getElementById('tab-warning-count').textContent = n;
  document.getElementById('tab-warning-modal').classList.add('show');
}

function autoSubmitForTabSwitch() {
  closeModal('tab-warning-modal');
...
async function changeAdminCredentials() {
  ...
  if (!(await passwordMatches(current, getAdminPassword()))) {
  ...
  settings.adminAuth = {
    ...
    password: newPassword ? await hashPassword(newPassword) : getAdminPassword()
  };
  localStorage.setItem(SK_SETTINGS, JSON.stringify(settings));
...
// Shows total visit count + the 10 most recent timestamps under
// Settings -> Site Visits. Recorded once per page load by logSiteVisit().
function renderSiteVisits() {
  const countEl = document.getElementById('site-visits-count');
  const listEl = document.getElementById('site-visits-list');
  ...
  if (!visits.length) {
    listEl.innerHTML = '<div class="setting-hint">No visits recorded yet.</div>';
    return;
  }
  const recent = visits.slice(-10).reverse();
  listEl.innerHTML = recent.map(v => {
    const d = new Date(v.time);
    const label = isNaN(d.getTime()) ? v.time : d.toLocaleString();
    return `<div class="setting-hint">🕒 ${label}</div>`;
  }).join('');
}
